...
// Import the context without causing circular dependencies
const TranslationContext = require('@/contexts/TranslationContext').TranslationContext;

export function SafeTranslatedText({ translationKey, fallback }: { translationKey: string; fallback: string }) {
  try {
    const context = useContext(TranslationContext);
    if (context?.t) {
      const translated = context.t(translationKey);
...
export function SafeHeroSection({ totalUsers, currentYear }: SafeHeroSectionProps) {
  return (
    <div className="text-center mb-16">
      ...
      <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-semibold mb-4 sm:mb-6 bg-gradient-to-r from-white to-[#a4a5b0] bg-clip-text text-transparent leading-tight py-2">
        <SafeTranslatedText 
          ...
          fallback="Best Bitcoin Casino Bonuses & Crypto Promotions 2025" 
        />
      </h2>
      
      <p className="text-[#a4a5b0] max-w-3xl mx-auto text-base sm:text-lg md:text-xl leading-relaxed mb-6 sm:mb-8">
        <SafeTranslatedText 
          translationKey="homepage.description" 
          fallback={`Discover the best Bitcoin casino bonuses and promotional bonuses in ${currentYear}. Our expertly curated list includes trusted crypto casinos offering generous welcome packages, exclusive bonus codes, and free spins.`}
        />
      </p>
