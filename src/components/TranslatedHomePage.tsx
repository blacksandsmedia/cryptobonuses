...
export default function TranslatedHomePage() {
    ...
    const fetchData = async () => {
      try {
        ...
        const data = await response.json();
                 ...
                 // Extract bonuses with casino info and convert to CasinoBonus format
  ...
  const filteredBonuses = bonuses.filter(bonus => {
    ...
    if (filters.sortBy === 'newest') return 0; // Would need dates
    if (filters.sortBy === 'oldest') return 0; // Would need dates
        ...
        <div className="mt-24 mb-16">
          ...
          <div className="text-center mb-16">
            <div className="inline-flex items-center gap-2 bg-[#68D08B]/10 border border-[#68D08B]/20 rounded-full px-6 py-3 mb-6">
              <div className="w-2 h-2 bg-[#68D08B] rounded-full animate-pulse"></div>
              <span className="text-[#68D08B] text-sm font-medium">
                {totalUsers >= 1000 ? `${Math.floor(totalUsers / 1000)}K+` : `${totalUsers}+`} Players {t('homepage.trustedBy')}
              </span>
            </div>
            ...
            <p className="text-[#a4a5b0] max-w-3xl mx-auto text-base sm:text-lg md:text-xl leading-relaxed mb-6 sm:mb-8">
              {t('homepage.welcomeDescription').replace('{year}', currentYear.toString())}
            </p>
